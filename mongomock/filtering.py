...
def filter_applies(search_filter, document):
    ...
    for key, search in iteritems(search_filter):
        ...
        if (
            ...
            and ("$ne" in search or search == {"$exists": False})
        ...
        for doc_val in iter_key_candidates(key, document):
            if isinstance(search, dict):
                is_match = (
                    all(
                        operator_string in OPERATOR_MAP
                        and OPERATOR_MAP[operator_string](doc_val, search_val)
                        or operator_string == "$not"
                        and _not_op(document, key, search_val)
                        for operator_string, search_val in iteritems(search)
                    )
                    and search
                ) or doc_val == search
            elif isinstance(search, RE_TYPE) and isinstance(
                doc_val, (string_types, list)
            ):
...
def _regex(doc_val, regex):
    ...
    return any(
        regex.search(item)
        for item in _force_list(doc_val)
